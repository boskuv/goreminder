...
### Fixed
- **PUT /api/v1/tasks/{id}**: editing title, description, or other schedule metadata on recurring tasks (`cron_expression` or `rrule`, `requires_confirmation=false`) with an overdue `start_date` now republishes `worker.schedule_task` with the next occurrence (same calculation as unmute/autoreschedule) so the worker stays in sync with the database; one-time tasks with a past `start_date` still skip queue publish on metadata-only updates.
...
### Added
- **GET /api/v1/messengerRelatedUsers/all**: optional query parameters `user_id` and `chat_id` to filter messenger links. Omitting them keeps the previous unfiltered list behavior.
...
- **Task attachments** (S3-compatible object storage; **attachment service** is a separate deployable, **core** is REST BFF):
  ...
  - **`attachments.purgeOnTaskDone`**: when `true` (and `attachments.enabled`), `POST /api/v1/tasks/{id}/done` triggers best-effort `PurgeByTask` for the task and its child tasks (default `false` keeps attachments on done tasks).
...
- **List/digest filters**: optional query parameter `messenger_user_id` on `GET /api/v1/users/{user_id}/tasks`, `GET /api/v1/backlogs`, `GET /api/v1/targets`, `GET /api/v1/digests`, and `GET /api/v1/digests/settings/all`. The external messenger identifier is resolved to internal `messenger_related_user_id` rows (scoped by `user_id` where applicable); empty result when no matching link exists.
