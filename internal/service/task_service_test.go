...
func setup(t *testing.T) (*TaskService, *mock_repositories.MockTaskRepository, *mock_repositories.MockUserRepository, *mock_repositories.MockMessengerRepository, *mock_repositories.MockTaskHistoryRepository, *queue.Producer) {
	...
	service := NewTaskService(taskRepo, userRepo, messengerRepo, taskHistoryRepo, producer, attachments.NewNoopClient(), NoopActivityTracker{}, false, testLogger)
...
func TestTaskService_GetUserTasks_FilterByMessengerRelatedUserID(t *testing.T) {
	...
	tasks, count, err := service.GetUserTasks(ctx, userID, page, pageSize, orderBy, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, &messengerRelatedUserID, nil, nil)
	assert.NoError(t, err)
	assert.Equal(t, expectedTasks, tasks)
	assert.Equal(t, totalCount, count)
}
...
func TestTaskService_GetUserTasks_MessengerRelatedUserIDWinsOverMessengerUserID(t *testing.T) {
	...
	tasks, count, err := service.GetUserTasks(ctx, userID, page, pageSize, orderBy, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, &messengerRelatedUserID, &messengerUserID, nil)
	assert.NoError(t, err)
	assert.Equal(t, expectedTasks, tasks)
	assert.Equal(t, totalCount, count)
}
